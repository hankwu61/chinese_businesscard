...
class QuickNERTrainer:
    ...
    def prepare_data(self, texts: list, labels: list, batch_size: int = 8):
        """準備數據"""
    ...
    def train(self, train_loader, val_loader, epochs: int = 3, learning_rate: float = 3e-5):
        """訓練模型"""
...
def main():
    ...
    # 生成較少的數據進行快速測試
    print("生成訓練數據...")
    texts, labels = processor.generate_dataset(num_samples=500)  # 減少樣本數量
    
    print(f"數據集大小: {len(texts)} 個樣本")
    
    # 初始化訓練器
    trainer = QuickNERTrainer()
    
    # 準備數據
    train_loader, val_loader = trainer.prepare_data(texts, labels, batch_size=8)  # 減少batch size
    
    # 快速訓練模型
    trainer.train(train_loader, val_loader, epochs=3, learning_rate=3e-5)  # 減少epochs
